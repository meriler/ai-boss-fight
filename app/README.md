...
- `reducer.js` — ЕДИНЫЙ редьюсер журнала (20 типов, словарь закрыт): живое применение
  ...
  (догоняющий/подсказка l3/рестарт run давали карточку «3 → 3 из 4»).
...
- `phases.js` — рендерер тактов по elements[] манифеста (identity-agnostic): корзины
  ...
  «← Назад» в тактах версии до коммита, полоска эпох у head при «Научить», разгадка —
  текст до картинки и «крупно один раз», путь детектива плашками из данных курса.
