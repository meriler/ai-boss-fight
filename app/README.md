...
- `/z1.html?solo=1` — SOLO: самостоятельный проход демки ОДНИМ человеком по ссылке, без
  ведущего и без серверного run'а (для методиста Сергея и кого угодно по ссылке).
  Полностью КЛИЕНТСКИЙ транспорт (`core/solo.js`): reveal решается локально («Показать
  разгадку» вместо ожидания N/N), seat-save и acked-коммиты в localStorage (F5 работает),
  `/sync` и `/host` не нужны, `/tele` остаётся fire-and-forget. Ни одного серверного
  вызова ядра — проход нигде не встаёт в ожидание. Движок НАСТОЯЩИЙ (не demo — щупает
  ровно то, что дети). Гейты без кода, разговорные моменты помечены «(в живом занятии —
  с ведущим)». Доказательство «не блокируется» — solo-прогон в `e2e/e2e-z1.mjs`.
